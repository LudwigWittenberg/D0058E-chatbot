"""
Model selection for the Chatbot App.
...
def get_available_backends() -> list:
    """
    Return list of available LLM backends with their status.

    Checks each known backend for availability by attempting a connection test.

    Returns:
        [{"name": str, "backend": str, "status": "available"|"unavailable"}]
    """
    backends = []

    for name, backend_enum in _BACKEND_MAP.items():
        entry = {
        ...
        try:
            if backend_enum == LLMBackend.OLLAMA:
                # Ollama can be tested without an API key
                config = LLMConfig(
                ...
                # OpenAI requires an API key — check if one is configured
                import os
                api_key = os.environ.get("OPENAI_API_KEY")
                if api_key:
                    entry["status"] = "available"
            elif backend_enum == LLMBackend.GEMINI:
                # Gemini requires an API key — check if one is configured
                import os
                api_key = os.environ.get("GEMINI_API_KEY")
                if api_key:
                    entry["status"] = "available"
        except Exception:
            # If anything goes wrong, the backend is unavailable
            pass
...
def create_client(backend: str, config: dict) -> LLMClient:
    ...
    api_key = config.get("api_key_ollama") or config.get("api_key_openai") or config.get("api_key_gemini") or config.get("api_key")
    if backend == "openai":
